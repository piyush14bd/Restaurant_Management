...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		matchStage := bson.D{{"$match", bson.D{{}}}} // different stages of mongo DB aggregations(huge topics small part of it) match stage is for matching a criteria.
		groupStage := bson.D{{"$group", bson.D{      //group using that criteria
			{"_id", bson.D{{"_id", "null"}}},
			{"total_count", bson.D{{"$sum", 1}}},
			{"data", bson.D{{"$push", "$$Root"}}},
		}}}
		projectStage := bson.D{ //allows you to define what all value should go to frontend.
			{"$project", bson.D{
				{"_id", 0},
				{"total_count", 1},
