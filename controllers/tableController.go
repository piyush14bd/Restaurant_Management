...
func GetTable() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while fetching the table items"})
		}
...
func UpdateTable() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		table.Updated_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
		updateObj = append(updateObj, bson.E{"update_at", table.Updated_at})

		upsert := true
		filter := bson.M{"table": tableId}
